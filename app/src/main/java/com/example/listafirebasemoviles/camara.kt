...
import kotlinx.android.synthetic.main.camara.*
import java.io.File
...
class camara : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        camaraboton.setOnClickListener {
            val takePictureIntent = Intent(MediaStore.ACTION_IMAGE_CAPTURE)
            fotofile = getfotofile(FILE_NAME)

            takePictureIntent.putExtra(MediaStore.EXTRA_OUTPUT, fotofile)
            val fileProvider = FileProvider.getUriForFile(this, "com.example.listafirebasemoviles.fileprovider", fotofile)
